...
import { uploadEventGalleryImage, type GalleryUploadState } from "./actions";

const initialState: GalleryUploadState = {};

export function AddGalleryImageForm({ eventId }: { eventId: string }) {
  const [isCompressing, setIsCompressing] = useState(false);
  const formRef = useRef<HTMLFormElement>(null);

  // See the identical comment on the vendor gallery's own copy of this
  // form: the compression step has to live inside the action passed to
  // useActionState itself, not in a wrapper that calls the returned
  // dispatcher manually — that breaks React's transition tracking for
  // `pending`.
  const [state, formAction, pending] = useActionState(async (_prevState: GalleryUploadState, formData: FormData) => {
    ...
    const result = await uploadEventGalleryImage(_prevState, formData);
