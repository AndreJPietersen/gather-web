...
async function displayNameFor(supabase: SupabaseClient, userId: string): Promise<string | null> {
...
export async function checkAndSendPaymentReminders(caller: Caller): Promise<void> {
...
export async function checkAndSendTaskReminders(caller: Caller): Promise<void> {
  const supabase = caller.supabase;
  const user = { id: caller.userId, email: caller.email };
  if (!user.email) return;
