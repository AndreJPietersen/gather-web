...
| `/admin/watchlist` | Early-warning report for marketplace flooding — never blocks anything. Five signals computed live by `admin_watchlist()` (service role only): listing bursts, listings sharing a phone/website, people on many vendor teams, claim sprees, invite floods. Thresholds in `src/lib/admin/watchlist.ts`. A banner on the Dashboard links here whenever anything is flagged. Per entry: **Hide these listings** (bursts and shared-contact groups), **Review / suspend user**, and **Dismiss** with a note (returns if the count grows; Undo list). |
| Moderation on `/admin/planners/[id]` and `/admin/vendors/[id]` | User page: **Suspend** (Supabase Auth ban + `user_suspensions` row; rejects their pending claims/business requests; optional hide of the listings they control) / **Lift suspension**; **Hide their listings**; **Remove** per vendor membership. Vendor page: **Hide listing** / **Restore listing** (`vendors.hidden_at`); **Remove** per team member. All audit-logged; shared logic in `src/lib/admin/moderation.ts`. |
