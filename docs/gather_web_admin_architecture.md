...
Instead: every `/admin/*` Server Component and Server Action starts by calling a `requireAdmin()` guard, and only after that passes does it use the **service-role client** (`apps/web/src/lib/supabase/service.ts`, already established in Phase 3) for all of its actual data access — bypassing RLS entirely, deliberately, the same way the guest-RSVP path already does for a much narrower case. This avoids policy sprawl and matches the Salesforce mental model more directly: an admin isn't "shared" a record through some grant, they simply aren't subject to the sharing model at all.
...
| `/admin/vendors/[id]` | One vendor's detail: services, team, quotes, event associations, case history. Also lists that vendor's featured placements and links to add one (see `/admin/featured` below). |
| `/admin/featured` | Paid featured placements: a live "order on the site right now" preview, then every placement grouped Live / Scheduled / Pending / Ended / Cancelled, with Activate / Cancel / Reopen / Edit. `/admin/featured/new` and `/admin/featured/[id]` hold the form: vendor, start and end date (inclusive, SAST), an optional pinned position (blank = rotating pool), status (pending / activated), an internal fee and note. Refuses two placements holding the same position over overlapping dates. Every change is written to `admin_audit_log`. Table: `vendor_feature_placements` (admin-only, service role; RLS on with no policies). Vendor-requested placements arrive here as Pending, labelled "Requested by the vendor" with their chosen spot, duration and note. The top of the page holds the **global on/off switch** (`app_settings.featured_enabled`) — off hides every featured badge/row/boost site-wide and stops requests, keeping placements intact. |
| `/admin/featured/pricing` | The featured price list shown to vendors (public `/featured` page and the request form): one amount per spot (Top / Rotating) × duration (1 week / 1 month / 3 months); blank = "Price on request". Table: `feature_prices`; audit-logged. |
| `/admin/emails` | Gather-branded email. **Templates** tab: every template (the four automatic reminder templates marked "Automatic" — editable wording, not deletable) with an editor at `/admin/emails/templates/[id]` (subject, preview text, heading, simple-text body or raw HTML, optional button, merge-field chips, live preview, Send test to me, Duplicate, Delete). **Sent** tab: every admin send with counts; `/admin/emails/sent/[id]` shows each recipient's status. `/admin/emails/send`: one person, a group (everyone / planners / vendor owners / a business's team / recent joiners), or typed addresses; check recipients, test, send (limit from Settings; "SEND" confirmation for more than one). Announcements carry an unsubscribe link and skip opted-out addresses. Tables: `email_templates`, `admin_emails`, `admin_email_recipients`, `email_suppressions` (all admin-only). |
| `/admin/settings` | Every operational knob, stored in the database and applied immediately: **sign-ups open/paused** (kill switch — trigger on `auth.users`), featured vendors on/off, **listing limits** (businesses one person can own; listings per 24h — trigger on `vendors`), all eight **watchlist thresholds**, and **hourly limits per person** for 21 kinds of write (`write_rate_limits`, enforced by `enforce_write_rate_limit`). Every change audit-logged. |
| `/admin/watchlist` | Early-warning report for marketplace flooding — never blocks anything. Five signals computed live by `admin_watchlist()` (service role only): listing bursts, listings sharing a phone/website, people on many vendor teams, claim sprees, invite floods. Thresholds in `apps/web/src/lib/admin/watchlist.ts`. A banner on the Dashboard links here whenever anything is flagged. Per entry: **Hide these listings** (bursts and shared-contact groups), **Review / suspend user**, and **Dismiss** with a note (returns if the count grows; Undo list). |
| Moderation on `/admin/planners/[id]` and `/admin/vendors/[id]` | User page: **Suspend** (Supabase Auth ban + `user_suspensions` row; rejects their pending claims/business requests; optional hide of the listings they control) / **Lift suspension**; **Hide their listings**; **Remove** per vendor membership. Vendor page: **Hide listing** / **Restore listing** (`vendors.hidden_at`); **Remove** per team member. All audit-logged; shared logic in `apps/web/src/lib/admin/moderation.ts`. |
| `/admin/vendors/requests` | Business exception requests: users who hit an owner rule (max 5 owned businesses, one per category) while adding a business. Each card shows the requested name/category, which rule(s) it needs lifted, the reason, and what the requester already owns. Approve (lets that one business be created through the normal form — spent on use) or Reject with an optional reason. Audit-logged. Table: `vendor_business_requests`. Pending count shown on the Dashboard and the Vendors header. |
